...
export class ResourceController extends BaseController {
  ...
  public async create(
    ...
    newResource.id = resourceInfo.id ? resourceInfo.id : this.tx.stub.generateUUID(JSON.stringify(resourceInfo))
    await this.createProvenanceInfo(newResource, currentSender, resourceInfo)
  ...
  public async update(
    ...
    console.log('==== resource info Updated ', resourceInfo)
    let currentSender = await this.validateCurrentSender()
    let lstResource = await Resource.query(Resource, {
      "selector": {
        type: new Resource().type,
